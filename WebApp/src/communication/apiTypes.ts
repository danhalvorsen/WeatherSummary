...
export class WeekNoValidator extends Validator<WeekNumber> {
  constructor() {
    super();
    this.ruleFor('value')
      .greaterThan(0)
...
export class BetweenTwoDatesValidator extends Validator<twoDates> {
  constructor() {
    super();
    this.ruleFor('from').setValidator(() => new MyDateValidator());
    this.ruleFor('to').setValidator(() => new MyDateValidator());
