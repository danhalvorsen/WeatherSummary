  ...
  return (
    <>
      <div className="d-inline-flex ms-5">
        ...
        <input type="checkbox" id="Today" checked={check} onClick={()=>{setCheck(!check); setActiveDate(!activeDate)}} />     <label htmlFor="Today"><p className="me-2">Today:</p></label> <strong>{todayDate}</strong> <br />
        <input type="checkbox" id="Choose" checked={!check} onClick={()=>{setActiveDate(!activeDate); setCheck(!check)}} />      <label htmlFor="Choose">Choose Dates:</label>
        </div>
      </div>
